...
program
  ...
  .option(
    "-r, --region <region>",
    "The region you would like to work in. Please keep in mind that the region must match the region of your space. You can use us, cn or eu, if left empty, default is eu. This region flag will be used for the other cli's commands"
  )
  .action(async (options) => {
    ...
    try {
      ...
      console.log(
        chalk.red("X") +
          " An error occurred when logging the user: " +
          e.message
      );
      process.exit(1);
    }
  ...
  .action(async () => {
    if (api.isAuthorized()) {
      try {
        ...
        console.log(
          chalk.green("✓") +
            ` Hi ${user.friendly_name}, you current logged in with: ${
              creds.get().email
            }`
        );
      } catch (e) {
        console.log(
          chalk.red("X") +
            ` Please check if your current region matches your user's region: ${e.message}.`
        );
      } finally {
        process.exit(0);
      }
...
program
  ...
  .action(async () => {
    try {
      await api.logout();
      console.log(
        "Logged out successfully! Token has been removed from .netrc file."
      );
      process.exit(0);
    } catch (e) {
      console.log(
        chalk.red("X") +
          " An error occurred when logging out the user: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      console.log(
        chalk.red("X") +
          " An error occurred when executing the pull-languages task: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    "--sf, --separate-files [value]",
    "Argument to create a single file for each component"
  )
  .option("-p, --path <path>", "Path to save the component files")
  .option(
    "-f, --file-name <fileName>",
    "custom name to be used in file(s) name instead of space id"
  )
...
program
  ...
  .action(async (component) => {
    ...
    try {
      ...
      console.log(
        chalk.red("X") +
          " An error occurred when executing the delete-component task: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .description(
    "Delete all components in your space that occur in your source file."
  )
  .option(
    "-r, --reverse",
    "Delete all components in your space that do not appear in your source.",
    false
  )
  ...
  .action(async (source, options) => {
    ...
    try {
      ...
      api.setSpaceId(space);
      await tasks.deleteComponents(api, {
        source,
...
program
  ...
  .action(async (name) => {
    ...
    try {
      ...
      console.log(
        chalk.green("✓") + " - views/components/_" + name + ".liquid"
      );
      console.log(
        chalk.green("✓") + " - source/scss/components/below/_" + name + ".scss"
      );
      process.exit(0);
    } catch (e) {
      console.log(
        chalk.red("X") +
          " An error occurred when executing operations to create the component: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .action(async () => {
    console.log(
      `${chalk.blue(
        "-"
      )} Select a boilerplate, fieldtype or theme to initialize\n`
    );
    ...
    try {
      ...
      console.error(
        chalk.red("X") +
          " An error ocurred when execute the select command: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    "--components-groups <UUIDs>",
    "Synchronize components based on their group UUIDs separated by commas"
  )
  .option(
    "--datasources-starts-with-slug <SLUG>",
    "Synchronize datasources that starts with the given slug"
  )
  .option(
    "--datasources-starts-with-name <NAME>",
    "Synchronize datasources that starts with the given name"
  )
  .action(async (options) => {
    ...
    try {
      ...
      console.log(
        "\n" +
          chalk.green("✓") +
          " Sync data between spaces successfully completed"
      );
    } catch (e) {
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(
        chalk.red("X") +
          " An error ocurred when execute quickstart operations: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .action(async (options) => {
    ...
    console.log(
      `${chalk.blue(
        "-"
      )} Creating the migration file in ./migrations/change_${component}_${field}.js\n`
    );
    ...
    try {
      ...
      console.log(
        chalk.red("X") +
          " An error ocurred when generate the migration file: " +
          e.message
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    ...
    "Publish the content. It can be: all, published or published-with-changes"
  )
  ...
  .action(async (options) => {
    ...
    if (publish && !publishOptionsAvailable.includes(publish)) {
      console.log(
        chalk.red("X") +
          " Please provide a correct publish option: all, published, or published-with-changes"
      );
      process.exit(1);
    }
    ...
    console.log(
      `${chalk.blue(
        "-"
      )} Processing the migration ./migrations/change_${component}_${field}.js\n`
    );
    ...
    try {
      ...
      api.setSpaceId(space);
      await tasks.runMigration(api, component, field, {
        isDryrun,
        publish,
        publishLanguages,
      });
      ...
      console.log(
        chalk.red("X") +
          " An error ocurred when run the migration file: " +
          e.message
      );
      process.exit(1);
    }
