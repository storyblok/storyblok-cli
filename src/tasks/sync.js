...
const SyncSpaces = {
  ...
  async updateStoriesAndFolders (data, targetContent = null, sourceContent = null, isFolder = false) {
    ...
    if (targetContent) {
      ...
      createdStory = await this.targetClient.put(`spaces/${this.targetSpaceId}/stories/${targetContent.id}`, payload)
    ...
    if (createdStory.uuid !== sourceContent.uuid) {
      await this.targetClient.put(`spaces/${this.targetSpaceId}/stories/${createdStory.id}/update_uuid`, { uuid: sourceContent.uuid })
    }
  ...
  async syncStories () {
    ...
    for (const story of allStories) {
      ...
      try {
        const { data } = await this.targetClient.get('spaces/' + this.targetSpaceId + '/stories', { with_slug: story.full_slug })
  ...
  async syncFolders () {
    ...
    for (const folder of sourceFolders) {
      try {
        ...
        const { data } = await this.targetClient.get(`spaces/${this.targetSpaceId}/stories`, { with_slug: folder.full_slug })
